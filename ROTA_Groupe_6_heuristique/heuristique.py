...
def violation_espacement(vol, donnees, solution, vols):
    ...
    for autre_vol in solution:
        autre_id_vol = autre_vol[1]
        autre_t = autre_vol[2]
        if autre_id_vol == id_vol:
            continue
        autre_destination = obtenir_destination(autre_id_vol, vols)
        if autre_destination == destination:
            ecart = abs(autre_t - t)
            if ecart < min_spacing:
                violations += (min_spacing - ecart)
    return violations
...
def fonction_evaluation(solution, donnees, lambda_espacement, lambda_utilisation, vols, profit_total=None):
    if profit_total is None:
        # Recalculer le profit total à partir de la solution
        profit_total = 0
        for _, id_vol, t in solution:
            for vol in vols:
                if vol["vol"] == id_vol:
                    profit_total += vol["profit"]
                    break
...
def voisinage_1(solution, planning, profit_total, donnees):
    ...
    slots_disponibles = copy.deepcopy(donnees["slots"])
...
def voisinage_1(solution, planning, profit_total, donnees):
    ...
    vols_a_retirer = random.sample(nouvelle_solution, k=1)
    ...
    for vol in vols_a_retirer:
        ...
        # Réaffectation dans un autre créneau
            ...
            if slots_disponibles[t_nouveau] > 0:
                for k in range(len(nouveau_planning)):
                    if all(nouveau_planning[k][t_nouveau + dt] == 0 for dt in range(duree)):
                        ...
                        nouveau_profit_total += destinations[destination]["profit"][t_nouveau]
...
if __name__ == "__main__":
    ...
    donnees = charger_donnees(instance_file)
